...
export function Dashboard() {
  ...
  useEffect(() => {
    ...
    if (isAdmin) {
      ...
      ]).then(([todayRecord, balances]) => {
         if (isMounted) {
            setPersonalStats(todayRecord || { status: 'absent' });
            setLeaveBalances(balances || []);
         }
      }).catch(err => console.warn('Dashboard personal data error:', err))
        .finally(() => { if (isMounted) setLoading(false); });
    }
      ...
      <section className="dashboard-hero">
              ...
              <>
                <div className="summary-stat">
                  <span className="stat-value font-mono">
                    {loading ? '—' : (personalStats?.status === 'present' ? 'Present' : personalStats?.status === 'leave' ? 'On Leave' : 'Absent')}
                  </span>
                  <span className="stat-label">Today's Status</span>
                </div>
                <div className="summary-stat">
                  <span className="stat-value font-mono">
                    {loading ? '—' : (personalStats?.check_in ? new Date(personalStats.check_in).toLocaleTimeString([], {hour: '2-digit', minute:'2-digit'}) : '--:--')}
                  </span>
                  <span className="stat-label">Check In</span>
                </div>
                <div className="summary-stat">
                  <span className="stat-value font-mono">
                    {loading ? '—' : (personalStats?.work_hours ? `${personalStats.work_hours.toFixed(1)}h` : '0h')}
                  </span>
        ...
        <motion.div variants={staggerItem} className="attention-ledger">
          <div className="ledger-header">
            <h3>{isAdmin ? 'Needs Attention' : 'My Overview'}</h3>
            <span className="badge badge-warning">{isAdmin ? 'Live Overview' : 'Personal'}</span>
          </div>
            ...
            {isAdmin ? (
              ...
              <>
                <motion.li variants={staggerItem} className="ledger-item">
                  <div className="ledger-meta">
                    <span className="font-mono text-sm">Attendance</span>
                    <span className="ledger-tag info">Daily</span>
                  </div>
                  <p className="ledger-text">Log your check-in and check-out times.</p>
                  <Magnetic strength={0.15}>
                    <button className="btn-link" onClick={() => navigate('/attendance')}>View Attendance</button>
                  </Magnetic>
                </motion.li>

                <motion.li variants={staggerItem} className="ledger-item">
                  <div className="ledger-meta">
                    <span className="font-mono text-sm">Time Off</span>
                    <span className="ledger-tag info">Leave</span>
                  </div>
                  <p className="ledger-text">
                    {leaveBalances.length > 0
                      ? `${leaveBalances[0].pending} pending requests, ${leaveBalances[0].total - leaveBalances[0].used} days remaining.`
                      : 'Manage your leave requests and balances.'}
                  </p>
                  <Magnetic strength={0.15}>
                    <button className="btn-link" onClick={() => navigate('/time-off')}>Request Leave</button>
                  </Magnetic>
                </motion.li>
                ...
                <motion.li variants={staggerItem} className="ledger-item">
                  <div className="ledger-meta">
                    ...
                    <span className="ledger-tag info">Dossier</span>
                  </div>
                  <p className="ledger-text">View your personal dossier and update information.</p>
                  <Magnetic strength={0.15}>
